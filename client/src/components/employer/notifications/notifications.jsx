...
class EmployerNotifications extends Component {
  constructor(props) {
    super(props);
  ...
  getProjects = async () => {
    if (!this.state.refreshing) {
      ...
      const { index, limit } = this.state;
      console.log("1");
      let res = await employerInstance.post("/myProjectsPopulatedBids");
...
class JobListing extends Component {
  ...
  custom_path = (item, status, block) => {
    if (block === "Creative Block") {
      return {
        // pathname: `/employer/creativeproject/${item._id}`,
  ...
  taskStatus = task => {
    {
      //check application status of each bid/selected:
      // finished += status >=4
      // bids += status <= 0
      // active += rest
      // check all 3 in case of creative block

      if (task.jobBlock === "Creative Block") {
        let finished = 0;
        let bids = 0;
        let active = 0;
        //none selected
        if (task.selected.length === 0) return task.bids.length + " Bids";
        else {
          bids = task.bids.length;
          task.selected.forEach(progress => {
            if (progress.status > 0)
              progress.status >= 4 ? finished++ : active++;
          });

          return `${bids} B : ${active} A : ${finished} F`;
        }
      } else {
        if (task.selected.length > 0)
          return task.selected[0].status >= 4 ? "Finished" : "Active";
        else
          return task.bids.length > 1
            ? task.bids.length + " Bids"
            : task.bids.length + " Bid";
      }

      // return task.status === "0" ? `${task.bids.length} Bids` : "Active";
    }
  };
        ...
        <div className="listings-container">
          {this.props.jobs.map((item, key) => (
            ...
            <Link
              key={item._id}
              className="listing"
              to={this.custom_path(item, item.status, item.jobBlock)}
            >
              ...
              <div className="listing-title">
                <h4>
                  {item.jobTitle}{" "}
                  <span className="listing-type">{this.taskStatus(item)}</span>
                </h4>
                <ul className="listing-icons">
                  <li>
                    <i className="fa fa-map-marker"></i> {item.jobLocation}
                  </li>
                  <li>
    ...
    return (
      <div className="job-filter">
        ...
        <div className="widget competences">
          ...
          <ul className="checkboxes" key="competences">
        ...
        <div className="widget">
          ...
          <ul className="checkboxes" key="rates">
            <li>
              <Form.Check
                key="hr"
                name="hr"
